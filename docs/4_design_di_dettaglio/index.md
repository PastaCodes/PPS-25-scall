...
* Uno o più:&ensp;$\mathrm{ALT}(E^+)=\mathrm{ALT}(E)\cdot\lbrace\, R \,\rbrace$,&ensp;$\mathrm{PROD}(E^+)=\mathrm{PROD}(E^\ast)$.
...
che vanno a sostituire le produzioni non fattorizzate $\lbrace\,X\rightarrow\alpha \mid \alpha \in A\,\rbrace$.
...
* **Fallback posizionale (Priorità)**: Se $\vert{}M_{max}\vert{} > 1$, si verifica una collisione fra terminali che riconoscono la medesima porzione di testo
  ...
  Definendo $idx(t)$ come l'indice di dichiarazione del terminale $t$, si estrae l'unico vincitore $(t^\ast, p_{t^\ast})$ tale che $idx(t^\ast)$ sia minimo.
* **Scarto dei terminali ignorabili**: Se il terminale vincitore $t^\ast$ è esplicitamente marcato come ignorabile (es. spaziature o commenti),
  il prefisso $p_{t^\ast}$ viene consumato dall'input $S$, ma il sistema scarta la porzione in modo silente senza emettere alcun token nella sequenza di output (FS13).
