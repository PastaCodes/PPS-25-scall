...
class LexerLongestPrefixMatchTest extends AnyFunSuite:
  ...
  test("Lexer recognize string"):
    ...
    tokens.collect { case Token.Valid(t, _) => t } shouldBe List(BasicGrammar.ifRule)
    tokens.map(_.lexeme) shouldBe List("if")

  test("Lexer recognize regex"):
    val tokens = basicLexer.tokenize("test").toList
    tokens.collect { case Token.Valid(t, _) => t } shouldBe List(BasicGrammar.idRule)
  ...
  test("Lexer should use declaration order as the tie-breaker mechanism"):
    ...
    basicTokens.collect { case Token.Valid(t, _) => t } shouldBe List(BasicGrammar.ifRule)
    basicTokens.map(_.lexeme) shouldBe List("if")

    val invertedToken = invertedLexer.tokenize("if").toList
    invertedToken.collect { case Token.Valid(t, _) => t } shouldBe List(InvertedGrammar.idRule)
    invertedToken.map(_.lexeme) shouldBe List("if")

    val regexTokens = regexLexer.tokenize("ab").toList
    regexTokens.collect { case Token.Valid(t, _) => t } shouldBe List(RegexGrammar.specificRegexRule)
  ...
  test("Lexer emits Token.Error for unrecognized characters and keeps scanning"):
    ...
    tokens.collect { case Token.Valid(t, _) => t } shouldBe List(BasicGrammar.numRule, BasicGrammar.idRule)
    tokens.collect { case Token.Error(s) => s } shouldBe List("$", "#")
